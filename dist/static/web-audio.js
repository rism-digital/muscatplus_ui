/*
 From https://github.com/hayleigh-dot-dev/elm-web-audio/blob/1.1.0/src/elm-web-audio.js
 */
...
class VirtualAudioGraph {
    ...
    static prepare(graph = []) {
        ...
        const key = (graph, base = '') => {
            graph.forEach((node, i) => {
                ...
                // Recursively assign keys to the node connections.
        ...
        // however, so the next step in preparation is to flatten the graph
        // into a single array.
    ...
    constructor(context = new AudioContext(), opts = {}) {
        // Borrowing a convention from virtual dom libraries, the $ sign //is used to
        ...
        this.$nodes = {}
        // We keep track of the previous graph so that we can perform a diff and work out
    ...
    // basically exists so developers don't have to reach in and
