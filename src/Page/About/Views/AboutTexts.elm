...
aboutTextGerman : String
aboutTextGerman =
    """# Über RISM Online
Das Répertoire International des Sources Musicales (RISM) - Internationales Quellenlexikon der Musik ist eine
internationale, gemeinnützige Organisation, die sich zum Ziel gesetzt hat, weltweit vorhandene musikalische Quellen
...
durch umfangreiche Such- und Browse-Funktionen. Mehrere Einstiegspunkte mit verschiedenen Suchmasken für Quellen,
Personen und Institutionen bieten den Nutzern die Möglichkeit, relevante Ergebnisse mittels verschiedener Filter
einfach zu verfeinern, indem sie mit unterschiedlichen weiteren Informationen, z. B. Stichworte oder
Beziehungsinformationen, verknüpft werden.

Die Incipit-Suchmaske ermöglicht eine schnelle und genaue Suche nach Notationen, wobei die Teile des Incipits, die
einer Suchanfrage entsprechen, durch farbige Noten hervorgehoben werden. Benutzer können auch Incipits eingrenzen,
die übereinstimmen, indem sie einen Notenschlüssel, eine Tonart oder eine Taktart als Filter angeben oder eine
Notationssuche mit einer Stichwortsuche nach Komponistennamen oder Werken kombinieren.

Der Suchfilter "Nationale Sammlung" ermöglicht es den Benutzern, alle Suchfunktionen auf die Quellenlage eines
einzelnen Landes zu beschränken. Dadurch werden die Suchmöglichkeiten automatisch auf die in diesem Land verfügbaren
Quellen, Institutionen und Musikincipits eingegrenzt. Da Personen mehrere Länder umfassen können, werden diese von
den Suchfunktionen ausgeschlossen, wenn eine spezifische nationale Sammlung aktiv ist.

Die URLs sind in RISM Online verbindlich, stabil und zitierfähig. Sie können die URLs mit Lesezeichen versehen,
zitieren und mit Ihren Kollegen teilen. Wir stehen mit unserer institutionellen Garantie dafür ein, dass die
URLs persistent bleiben. Dazu gehören auch Links zu Suchvorgängen und Vorschauen, die es den Benutzern ermöglichen,
ihre komplexen Suchanfragen mit der Gewissheit weiterzugeben, dass der Empfänger so gut wie möglich dieselbe Ansicht
sehen wird.

## Open Data

Sämtliche Daten in RISM Online sind unter einer Creative Commons CC-BY 3.0 Lizenz verfügbar. Es steht Ihnen frei,
...
Wir arbeiten stetig an der Verbesserung von RISM Online und freuen uns über Rückmeldungen zu Ihren Erfahrungen!
...
Allgemeine Rückmeldungen können an [feedback@rism.online](mailto:feedback@rism.online) gesendet werden.

Alle Datensätze enthalten einen Link "Problem melden" am unteren Rand der jeweiligen Seite, der Sie zu einem
Formular führt, in dem Sie ein Problem beschreiben können. Wann immer möglich, ist es hilfreich, einen Link zu dem
Datensatz oder der Suche anzugeben, der/die das Problem ausgelöst hat, damit wir es nachvollziehen können. Die URL
der aktuellen Seite wird automatisch in das Feedback-Formular eingefügt.
...
**Incipits**: Sarasin, Lukas. [1760–1802]. **Katalog der Lucas Sarasin'schen Musiksammlung**. Universitätsbibliothek
Basel, HKun d III 9. Public Domain. [e-Manuscripta.](https://doi.org/10.7891/e-manuscripta-13836).
"""


aboutTextItalian : String
aboutTextItalian =
    """# A proposito di RISM Online
Il Répertoire International des Sources Musicales (RISM) è un'organizzazione internazionale no-profit che si propone
...
a queste fonti musicali attraverso estese funzionalità di ricerca e navigazione. Diversi punti di accesso, con
interfacce di ricerca dettagliate per fonti, persone e istituzioni, forniscono agli utenti la possibilità di ottenere
...
Il filtro di ricerca per collezione nazionale consente agli utenti di limitare tutte le funzionalità di ricerca alla
bibliografia di un singolo paese. Questo riduce automaticamente la ricerca alle sole fonti, istituzioni e incipit
musicali disponibili in quel paese. (Poiché le persone possono includere più paesi, vengono escluse dalle funzioni di
ricerca quando è attiva una collezione nazionale.)

Gli URL in RISM Online danno garanzia di autorevolezza, stabilità e citabilità. È possibile aggiungere un segnalibro,
...
condividere le proprie query, anche le più complesse, con la consapevolezza che il destinatario vedrà, nei limiti del
...
Ogni feedback generico può essere inviato all’indirizzo [feedback@rism.online](mailto:feedback@rism.online).

Tutte le schede presentano anche il collegamento "Segnala un problema" in fondo alla pagina, che ti porta a un modulo
in cui puoi descrivere il problema. Quando possibile, è utile includere un collegamento alla scheda o alla ricerca che
...
**Istituzioni**: Iliff, David. 2015. **L’interno della Duke Humphrey's Library, la più antica sala di lettura della
Bodleian Library all’Università di Oxford**. Licenza: CC BY-SA 3.0.
[Wikimedia Commons.](https://en.wikipedia.org/wiki/File:Duke_Humfrey%27s_Library_Interior_5,_Bodleian_Library,_Oxford,_UK_-_Diliff.jpg)

**Incipit**: Sarasin, Lukas. [1760–1802]. **Catalogo della collezione musicale di Lucas Sarasin**. Universitätsbibliothek
Basel, HKun d III 9. Public Domain. [e-Manuscripta.](https://doi.org/10.7891/e-manuscripta-13836)
"""


aboutTextFrench : String
aboutTextFrench =
    """# A propos de RISM Online
Le Répertoire International des Sources Musicales (RISM) est une organisation internationale à but non lucratif qui
vise à documenter de manière exhaustive les sources musicales existantes dans le monde entier : manuscrits, éditions
musicales imprimées, écrits sur la théorie musicale et livrets qui se trouvent dans les bibliothèques, les archives,
les églises, les écoles et les collections privées.

RISM Online, un service du [RISM Digital Center](https://rism.digital), permet d'accéder à ces sources musicales grâce à des fonctions de
recherche et de navigation étendues. Des points d'entrée multiples, avec des interfaces de recherche étendues sur les
sources, les personnes et les institutions, permettent aux utilisateurs de trouver rapidement les résultats pertinents
en utilisant plusieurs approches différentes, y compris des recherches par mots-clés, des filtres et des informations
sur les relations.

L'interface de recherche par incipit permet des recherches rapides et précises, avec, dans les résultats, la mise en
évidence en couleur des notes de l'incipit qui correspondent à la requête. Les utilisateurs peuvent également
restreindre les incipits qui correspondent en spécifiant une clef, une signature de tonalité ou une signature
temporelle comme filtre, ou combiner une recherche de notation avec une recherche par mot-clé pour les noms de
compositeurs ou d'œuvres.

Le filtre de recherche par collection nationale permet aux utilisateurs de restreindre toutes les fonctionnalités de
recherche au données d'un seul pays. Cela réduit automatiquement les possibilités de recherche aux seules sources,
institutions et incipits musicaux disponibles dans ce pays. (Étant donné que les personnes peuvent couvrir plusieurs
pays, elles sont exclues des fonctions de recherche).

## Open Data

Toutes les données de RISM Online sont disponibles sous une licence Creative Commons CC-BY 3.0. Vous êtes libre de
...
Nous améliorons constamment RISM Online. Vos commentaires sur votre expérience sont les bienvenus ! Faites-nous
savoir si vous n'avez pas trouvé ce que vous cherchiez, ou si certaines informations sont incorrectes ou manquantes.

Les commentaires généraux peuvent être envoyés à [feedback@rism.online](mailto:feedback@rism.online).

Toutes les fiches comportent également un lien "Report a problem" en bas de page, qui vous permet d'accéder à un
formulaire dans lequel vous pouvez décrire le problème. Dans la mesure du possible, il est utile d'inclure un lien
vers la notice ou la recherche qui correspond au problème afin que nous puissions le reproduire. L'URL de la page
actuelle est automatiquement ajoutée à la soumission du formulaire de retour d'information.
...
**Incipits**: Sarasin, Lukas. [1760–1802]. Katalog der Lucas Sarasin'schen Musiksammlung. Universitätsbibliothek Basel,
