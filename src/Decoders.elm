...
import DataTypes exposing (BasicSourceBody, Exemplar, ExemplarList, ExternalResource, ExternalResourceList, Facet, FacetItem(..), FacetList, FestivalBody, IncipitFormat(..), IncipitList, IncipitRecord, InstitutionBody, LabelValue, MaterialGroup, MaterialGroupList, NoteList, PersonBody, PersonNameVariantList, PersonSources, PlaceBody, RecordType(..), RelatedEntity, RelatedList, Relations, Relationship, RelationshipList, Relationships, RenderedIncipit(..), SearchBody, SearchPagination, SearchResult, SeeAlso, ServerResponse(..), SourceBody, Subject, recordTypeFromJsonType)
...
incipitDecoder : Decoder IncipitRecord
incipitDecoder =
    Decode.succeed IncipitRecord
        |> required "id" string
        |> required "label" labelDecoder
        |> required "summary" (list labelValueDecoder)
